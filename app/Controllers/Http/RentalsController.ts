...
export default class RentalsController {
    ...
    public async rent({auth , request , response , session}:HttpContextContract){
        ...
        const passcode = "bfb279f9aa9bdbcf158e97dd71a467cd2e0c893059b10f78e6b72ada1ed2c919"
        const shortCode = "174379"
        const timeStamp = moment().format("YYYYMMDDHHmmss")
        const password = Buffer.from(shortCode+passcode+timeStamp).toString("base64")
        const reqBody = {    
        ...
        try{
            const token = fs.readFileSync('token.txt').toString()
             await axios.post("https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest" , reqBody , {
                headers:{
            ...
            const rentalPayload = await request.validate({schema:rentalCreateSchema})
            const bike = await Bike.find(bikeId)
            ...
            .table('rentals')
            .insert({user_id:userId , bike_id:bikeId , rental_start:rentalPayload.rental_start.toJSDate() , rental_end:rentalPayload.rental_end.toJSDate() })
            // await Rental.create({
            //     user_id:userId,
            //     bike_id:bikeId,
